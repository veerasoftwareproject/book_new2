...
$qr="select *  from tblattend where DATE_FORMAT(created_at,'%Y-%m-%d')='$created_at'";
// echo$qr;
$qr=mysqli_query($link,$qr);		
$count=0;			
while($dt=mysqli_fetch_array($qr)){  $count++;?>                                  

                                        <tbody>
                                            <tr id="">
                                                <td><?php echo$dt['flno']; ?></td>
                                                <td><?php echo$dt['fname']; ?></td>
                                                <td><?php echo$dt['fphone']; ?></td>
                                                <td><?php echo$dt['attend_entry']; ?></td>
												<!-- <td >class="btnSelect_1"><span style="cursor:pointer; color:green;" class="material-symbols-outlined">check_box</span></td> -->
												<td><input type="text" name="" value='<?php echo$dt['remark']; ?>	' placeholder="Type here."></td>
												<!-- <td class="btnSelect_2"><span style="cursor:pointer; color:red; font-style:bold;" class="material-symbols-outlined">cancel</span></td> -->
												<!-- <td><?php echo$dt['gender']; ?></td> -->
												<td><select> <option value="">-Select-</option> <option value="present">Present</option> <option value="absent">Absent</option> </select></td>
												<td class="btnSelect"><span style="cursor:pointer;" class="material-symbols-outlined">edit_square</span></td>
                                            </tr>
                                        </tbody>  <?php   }  ?>
                                    </table>
                                </div>
...
<script>
 ...
 <script>
	$(document).ready(function(){
		$(".btnSelect_1").click(function(){
		var currentRow = $(this).closest("tr");
		var flno=currentRow.find("td:eq(3)").html();
		var attend_entry = "present";
		var fname = currentRow.find("td:eq(2)").html();
		var fphone = currentRow.find("td:eq(8)").html();
		$(this).closest("tr").remove();
		// document.getElementById('example').deleteRow(currentRow);
		$.ajax({
			url: "attend_data.php",
			method: "GET",
			data:{flno:flno, attend_entry:attend_entry, fname:fname, fphone:fphone},
			success: function(response){
				alert("ajax-1 working");
			}
		});
		});


		$(".btnSelect_2").click(function(){
		var currentRow = $(this).closest("tr");
		var remark = currentRow.find("td:eq(5) input").val();
		// alert("hi::"+remark);
		var flno=currentRow.find("td:eq(3)").html();
		var attend_entry = "absent";
		var fname = currentRow.find("td:eq(2)").html();
		var fphone = currentRow.find("td:eq(8)").html();
		$(this).closest("tr").remove();
		$.ajax({
			url: "attend_data_2.php",
			method: "GET",
			data:{flno:flno, attend_entry:attend_entry, fname:fname, fphone:fphone, remark:remark},
			success: function(response){
				alert("ajax-2 working");
			}
		});
		});
...
<script>
	$(document).ready(function(){
		$(".btnSelect").click(function(){
			var currentRow =  $(this).closest("tr");
			var flno = currentRow.find("td:eq(0)").html();
			var remark = currentRow.find("td:eq(4) input").val();
			// var 
			var attend_entry = currentRow.find("td:eq(5) option:selected").val();
			$.ajax({
				url:"update_attend.php",
				method:"GET",
				data:{flno:flno, attend_entry:attend_entry, remark:remark},
				success: function(response){
					alert("AJAX working");
				}
				// window.location.href='attend_date_edit.php';
			});
		});
	});
</script>